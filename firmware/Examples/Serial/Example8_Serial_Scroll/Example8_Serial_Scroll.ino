/*
 ...
 This example shows how to shift the contents of the display to the right/left as well as change the cursor position
 by one spot to the right/left. These are low level commands of the HD44780. 
 ...
 Note: Because shifting the display is a low level command OpenLCD is not tracking the cursor position nor wrapping. 
 There are better ways to do this in software on the client side.
...
void shiftLeft()
{
  ...
  OpenLCD.write( (1<<4) | (1<<3) ); //Shift entire display to the left one
}

void shiftRight()
{
  OpenLCD.write(254); //Send command character
  OpenLCD.write( (1<<4) | (1<<3) | (1<<2) ); //Shift entire display to the right one
}

void writeForwards()
{
  OpenLCD.write( (1<<4) | (1<<2) ); //Cursor move to the right
  OpenLCD.write('!'); //Cursor is then advanced to the right
}

void writeBackwards()
{
  OpenLCD.write(254); //Send command character
  OpenLCD.write( (1<<4) ); //Cursor move to the left

  OpenLCD.write(254); //Send command character
  OpenLCD.write( (1<<4) ); //Cursor move to the left

  OpenLCD.write('x'); //The cursor then advances one position
}
